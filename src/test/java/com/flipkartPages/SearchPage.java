...
public class SearchPage extends PageObject {
	...
	@FindBy(xpath = "//section//div[contains(.,'Availability')]")
	...
	public int getProductPrice(WebElement mobile) {
		return Integer.parseInt(mobile.findElement(By.xpath("//div[@class='_30jeq3 _1_WHN1']")).getText()
				.replaceAll("[^\\dA-Za-z ]", ""));
	}

	public List<String[]> getResultDetails() {
		wait = new WebDriverWait(driver, 3);
		int totaldevices = Integer
				.parseInt(driver.findElement(By.xpath("//span[@class='_10Ermr']")).getText().split(" ")[5]);
		...
		do {
			...
			for (int i = 0; i < mobiles.size(); i++) {
				String priceToCompare = driver
						.findElement(
								By.xpath("(//a[@class='_1fQZEK']//div[@class='_30jeq3 _1_WHN1'])[" + (i + 1) + "]"))
						.getText().replaceAll("[^\\dA-Za-z ]", "");
				if (Integer.parseInt(priceToCompare) <= 40000) {
					String deviceDetails = driver
							.findElement(By.xpath("(//a[@class='_1fQZEK']//div[@class='_4rR01T'])[" + (i + 1) + "]"))
							.getText();
					String price = driver
							.findElement(
									By.xpath("(//a[@class='_1fQZEK']//div[@class='_30jeq3 _1_WHN1'])[" + (i + 1) + "]"))
							.getText();
					String reviews = driver
							.findElement(By.xpath("(//a[@class='_1fQZEK']//span[@class='_2_R_DZ'])[" + (i + 1) + "]"))
							.getText().split(" ")[0];
			...
			if (driver.findElements(By.xpath("//a[contains(.,'Next')]")).size() > 0) { // "//a[contains(.,'Next')]")
				...
				wait.until(
						ExpectedConditions.invisibilityOfElementLocated(By.xpath("//div[@class='_1XXPTY _1v2cG7']")));
				try {
					Thread.sleep(2000);
